...
export function Form_Agendamento_Funcionario({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isNovoCliente) {
      ...
      if (telefoneNovo.length !== 7)
        return alert("O telefone deve ter exatamente 7 dígitos.");
    } else {
  ...
  const handleFuncionarioChange = (id: string) => {
    ...
    // Procuramos o objeto do profissional selecionado na nossa lista
    const profSelecionado = profissionais.find((p) => String(p.id) === id);

    // Se o profissional tiver um servico_id vinculado a ele no banco de dados
    if (profSelecionado && profSelecionado.servico_id) {
      setServicoId(String(profSelecionado.servico_id));
    }
  };
  ...
  return (
    <div className="max-w-5xl w-full mx-auto bg-white rounded-[3rem] p-8 md:p-12 shadow-2xl border border-gray-100 animate-in fade-in zoom-in duration-300">
      <div className="flex justify-between items-center mb-10">
        <button
          onClick={onVoltar}
          className="text-gray-400 hover:text-black transition-colors flex items-center gap-2 font-bold uppercase text-[10px]"
        >
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-2">
            ...
            <select
              required
              className="w-full p-4 bg-gray-50 rounded-2xl font-bold outline-none border-none focus:ring-2 focus:ring-[#b5820e]"
              value={servicoId}
        ...
        <button
          type="submit"
          disabled={loading}
          className="w-full py-6 bg-black text-[#b5820e] rounded-[2rem] font-black uppercase tracking-[0.2em] shadow-2xl hover:scale-[1.01] active:scale-95 transition-all disabled:opacity-50"
        >
