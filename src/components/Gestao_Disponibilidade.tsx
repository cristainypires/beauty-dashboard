...
export function Gestao_Disponibilidade() {
  ...
  const [activeTab, setActiveTab] = useState<"semanal" | "bloqueio" | "ferias">(
    "semanal",
  );
  ...
  const carregarAgendaDoProfissional = async () => {
    ...
    try {
      const dadosBanco =
        await FuncionarioService.obterDisponibilidade(selectedProfId);
      const mapaDiasInverso = [
        "Segunda-feira",
        "Terça-feira",
        "Quarta-feira",
        "Quinta-feira",
      ...
      const novaSemana = semana.map((diaLocal) => {
        const diaNoBanco = dadosBanco.find(
          (d: any) => mapaDiasInverso[d.dia_semana] === diaLocal.dia,
        );
        return diaNoBanco
          ? {
              ...diaLocal,
  ...
  const handleSalvarSemanal = async () => {
    setLoading(true);
    try {
      await FuncionarioService.marcarDisponibilidade({
  ...
  const handleMarcarFerias = async () => {
    if (!ferias.data_inicio || !ferias.data_fim)
      return alert("Preencha o período!");
    setLoading(true);
    try {
      await FuncionarioService.marcarFerias({
        ...ferias,
        funcionario_id: selectedProfId,
      });
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-8 rounded-[2.5rem] shadow-sm border border-gray-100">
        <div className="mb-8">
          <h1 className="text-3xl md:text-4xl font-black uppercase tracking-tighter leading-tight">
            Gestão de <span className="text-[#b5820e]">Disponibilidade</span>
          </h1>
          <p className="text-gray-500 text-sm md:text-base font-bold uppercase mt-2 tracking-wide">
            Configurações de Agenda do Profissional ✏️
          </p>
        </div>
      ...
      {!selectedProfId ? (
        <div className="text-center py-20 bg-gray-50 rounded-[3rem] border-2 border-dashed border-gray-200">
          ...
          <p className="text-gray-400 font-bold uppercase text-sm tracking-widest">
            Selecione um profissional para gerir a agenda.
          </p>
        </div>
      ) : (
        <div className="space-y-6 animate-in fade-in duration-500">
          ...
          <div className="flex flex-wrap gap-2 p-1 bg-gray-100 rounded-3xl w-fit">
            <TabBtn
              active={activeTab === "semanal"}
              onClick={() => setActiveTab("semanal")}
              icon={<Clock size={16} />}
              label="Agenda Semanal"
            />
            <TabBtn
              active={activeTab === "bloqueio"}
              onClick={() => setActiveTab("bloqueio")}
              icon={<Lock size={16} />}
              label="Bloquear Horário"
            />
            <TabBtn
              active={activeTab === "ferias"}
          ...
          {activeTab === "semanal" && (
            <div className="space-y-4">
              {semana.map((d, i) => (
                <div
                  key={d.dia}
                  className={`p-6 rounded-[2rem] border flex flex-col md:flex-row justify-between items-center transition-all duration-300 
          ${!d.ativo ? "bg-gray-50 opacity-40 border-gray-100" : "bg-white shadow-lg border-gray-200 hover:shadow-xl"}
        `}
                >
                  ...
                  <div className="flex items-center gap-4 w-full md:w-1/3">
                    <input
                      ...
                      className="w-6 h-6 accent-[#b5820e] cursor-pointer"
                    />
                    <span className="font-black text-xl md:text-xl uppercase tracking-tighter">
                      {d.dia} {d.ativo ? "🟡" : "⚪"}
                    </span>
                  </div>
                  ...
                  {d.ativo && (
                    <div className="flex items-center gap-4 mt-4 md:mt-0 w-full md:w-2/3 justify-end">
                      <input
                        type="time"
                        value={d.entrada}
                        onChange={(e) => {
                        ...
                        className="p-3 md:p-4 bg-gray-100 rounded-xl font-bold border border-gray-200 focus:ring-2 focus:ring-[#b5820e]"
                      />
                      <ChevronRight size={24} className="text-[#b5820e]" />
                      <input
                        type="time"
                        value={d.saida}
                        onChange={(e) => {
              ...
              <SaveButton
                onClick={handleSalvarSemanal}
                loading={loading}
                text=" Atualizar Agenda Semanal"
              />
            </div>
          )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div>
                  ...
                  <input
                    ...
                    value={bloqueio.hora}
                    onChange={(e) =>
                      setBloqueio({ ...bloqueio, hora: e.target.value })
                    }
                    className="w-full p-4 bg-gray-50 rounded-2xl font-bold"
                  />
                </div>
            ...
            <div className="bg-black text-white p-10 rounded-[2.5rem] shadow-2xl space-y-8">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="text-xl font-black uppercase italic">
                    Marcar Período de Ausência
                  </h3>
                  <p className="text-gray-400 text-sm">
                    O profissional não aparecerá na lista de agendamentos nestas
                    datas.
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase text-[#b5820e]">
                    Data de Início
                  </label>
                  <input
                    type="date"
                    ...
                    onChange={(e) =>
                      setFerias({ ...ferias, data_inicio: e.target.value })
                    }
                    className="w-full p-5 bg-zinc-900 border border-zinc-800 rounded-2xl font-bold text-white focus:border-[#b5820e] outline-none"
                  />
                </div>
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase text-[#b5820e]">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setFerias({ ...ferias, data_fim: e.target.value })
                    }
                    className="w-full p-5 bg-zinc-900 border border-zinc-800 rounded-2xl font-bold text-white focus:border-[#b5820e] outline-none"
                  />
                </div>
              ...
              >
                {loading
                  ? "Processando..."
                  : "Registrar Período de Folga / Férias"}
              </button>
            </div>
          )}
        </div>
